...
The AI will pause and ask you for these. None of them require a Spotify account beyond what you already have:

1. **Where to clone the repo.** Default: `~/git/music-data` or wherever you keep code.
2. **Spotify Developer App credentials.** If you don't have an app yet, the AI walks you through registering one at https://developer.spotify.com/dashboard. The redirect URI must be exactly `http://127.0.0.1:8888/callback` — Spotify rejects `localhost` since their April 2025 policy update.
3. **Where your Spotify data dump lives.** You request it from https://www.spotify.com/account/privacy ("Extended streaming history" — Spotify says up to 30 days; in practice, it often arrives in a few). Point the AI at the unzipped folder when it's ready.
4. **How chatty you want it.** Some users want the AI to narrate every command; others want "just do it, tell me when it's done." Tell it which up front.
...
6. Run a smoke-test enrichment: `python -m scripts.enrich --tracks --min-plays 100 --rate-interval 2.0` — about 25 tracks, ~1 minute, zero rate-limit risk.
...
- Decide whether to scale enrichment beyond the smoke test. The full top-tier (`--min-plays 20`, ~40 minutes throttled) is a real API workload — your AI shouldn't auto-launch it without your explicit go-ahead.
